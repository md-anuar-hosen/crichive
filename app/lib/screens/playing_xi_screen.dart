...
class _PlayingXiScreenState extends ConsumerState<PlayingXiScreen> {
  ...
  Widget build(BuildContext context) {
    final squad = ref.watch(squadProvider((teamId: widget.teamId, tournamentSlug: widget.tournamentSlug)));
    ...
    return Scaffold(
      ...
      body: AsyncValueView(
        value: squad,
        onRetry: () => ref.invalidate(squadProvider((teamId: widget.teamId, tournamentSlug: widget.tournamentSlug))),
        data: (context, players) {
          final requiredCount = tournament.valueOrNull?.rules?.playersPerSide ?? 11;
          return Column(
            children: [
              Padding(
                padding: const EdgeInsets.all(16),
                child: Text('Select $requiredCount players (${_selected.length}/$requiredCount)'),
              ),
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return CheckboxListTile(
                      value: checked,
                      title: Text(p.name),
                      subtitle: Text([p.batting, p.bowling].whereType<String>().join(' · ')),
                      onChanged: (v) {
                        setState(() {
                          if (v == true) {
                            if (_selected.length < requiredCount) _selected.add(p.id);
                          } else {
              ...
              if (_selected.length == requiredCount) _CaptainKeeperPicker(
                players: players.where((p) => _selected.contains(p.id)).toList(),
              ...
              Padding(
                padding: const EdgeInsets.all(16),
                child: FilledButton(
                  onPressed: (_selected.length == requiredCount && _captainId != null && _keeperId != null && !_submitting)
                      ? _submit
                      : null,
                  child: _submitting
  ...
  Future<void> _submit() async {
    ...
    try {
      await ref.read(apiClientProvider).setPlayingXi(
            widget.matchId,
