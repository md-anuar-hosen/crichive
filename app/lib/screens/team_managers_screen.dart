...
class TeamManagersScreen extends ConsumerWidget {
  ...
  Future<void> _grant(BuildContext context, WidgetRef ref) async {
    ...
    try {
      final manager = await ref
          ...
          .grantTeamManager(tournamentSlug, teamId, email: email);
      ref.invalidate(teamManagersProvider(_key));
      if (!context.mounted) return;

      final nameMatches =
          expectedName.isEmpty ||
          expectedName.toLowerCase() == manager.displayName.toLowerCase();
  ...
  Future<void> _revoke(
    ...
    try {
      await ref
          .read(apiClientProvider)
          .revokeTeamManager(tournamentSlug, teamId, membershipId);
      ref.invalidate(teamManagersProvider(_key));
    } on ApiException catch (e) {
      if (!context.mounted) return;
      ScaffoldMessenger.of(
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          final _ = await ref.refresh(teamManagersProvider(_key).future);
        },
