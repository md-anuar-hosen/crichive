          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(
                labelText: 'Logo URL (leave blank to remove)',
              ),
            ),
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                labelText: 'Organising club/association',
              ),
            ),
          ],
        ),
    ...
    try {
      await ref
          ...
          .updateTournamentBranding(
            ...
            organizerOrg: orgController.text.trim(),
          );
          ...
          actions: [
            if (isAuthed) ...[
              ...
              IconButton(
                ...
                onPressed: tournament.value == null
                    ? null
                    : () => _editBranding(context, ref, tournament.value!),
              ),
              IconButton(
                tooltip: 'Edit rules',
...
class _FixturesTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return RefreshIndicator(
      onRefresh: () async {
        final _ = await ref.refresh(fixturesProvider(slug).future);
      },
...
class _TeamsTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return RefreshIndicator(
      onRefresh: () async {
        final _ = await ref.refresh(teamsProvider(slug).future);
      },
      child: AsyncValueView(
        value: teams,
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                leading: CircleAvatar(
                  child: Text(team.label.isNotEmpty ? team.label[0] : '?'),
                ),
...
class _StandingsTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return RefreshIndicator(
      onRefresh: () async {
        final _ = await ref.refresh(standingsProvider(slug).future);
      },
      child: AsyncValueView(
        value: standings,
                            ...
                            .map(
                              (s) => DataRow(
                                cells: [
                                  ...
                                  DataCell(
                                    Text(s.netRunRate.toStringAsFixed(3)),
                                  ),
                                ],
                              ),
                            )
...
class _BracketTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return RefreshIndicator(
      onRefresh: () async {
        final _ = await ref.refresh(knockoutBracketProvider(slug).future);
      },
      child: AsyncValueView(
        value: bracket,
            ...
            children: [
              _BracketTeamLine(
                ...
                isWinner:
                    ...
                    match.teamA?.id == match.winnerTeamId,
              ),
              ...
              _BracketTeamLine(
                ...
                isWinner:
                    match.winnerTeamId != null &&
                    match.teamB?.id == match.winnerTeamId,
              ),
...
class _AwardsTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return RefreshIndicator(
      onRefresh: () async {
        final _ = await ref.refresh(awardsProvider(slug).future);
      },
      child: AsyncValueView(
        value: awards,
